...
KALISTOREF = "~/GenomeFiles/AGPv3/ZmaAGPv3.22_cdna_kallisto.idx"
HISAT2REFDIR = "~/GenomeFiles/AGPv3/Sequence/hisat2_zm3_index/genome_snp_tran_ercc"
FCOUNTREF = "~/GenomeFiles/Ensembl/Zmav3.gtf"

FCRESULT = "./featureCount_result/Round53.featureCount"
FCSUMMARY = "./featureCount_result/FC_summary_Round53.txt"
...
rule FASTQC_SRA:
    ...
    output:
        './trimmed_FASTQ/FASTQC/{sample}_1.trimmed_fastqc.html',
        './trimmed_FASTQ/FASTQC/{sample}_1.trimmed_fastqc.zip',
        './trimmed_FASTQ/FASTQC/{sample}_2.trimmed_fastqc.html',
        './trimmed_FASTQ/FASTQC/{sample}_2.trimmed_fastqc.zip',
    shell:
        "fastqc -t 2 {input} -o ./trimmed_FASTQ/FASTQC "
...
rule KALLISTO_ALN:
    ...
    shell:
        " kallisto quant -i {KALISTOREF} -t 2 -o {output} -b 20 {input[0]} {input[1]}"
...
rule HISAT_ALN:
    input:
        ...
        "./trimmed_FASTQ/{sample}_2.trimmed.fastq.gz",
    output:
        temp("./trimmed_FASTQ/{sample}_PE.bam"),
        "./trimmed_FASTQ/hisat2_summary_{sample}_PE.txt"
        
    shell:
        "hisat2 -x {HISAT2REFDIR} -p 4 -t -1 {input[0]} -2 {input[1]} 2>{output[1]} |samtools view -Sbo {output[0]} - "
...
rule FEATURE_COUNT:
    ...
    output:
        ...
        FCSUMMARY
    shell:
        "featureCounts -T 16 -p -a {FCOUNTREF} -o {output[0]} {input} 2> {output[1]}"
